 	...
 	DialogBox.prototype = {	
 		...
 		hide: function(element){
 			...
 			setTimeout(function(){
				...
				// 删除弹窗
				if($('#auto-close-dialogBox').length >= 1) {
					$('#auto-close-dialogBox').remove();
				}
 			},150)
 		},
...
// 					return $(document).width()*0.3;
	...
	$.alert = function(text,myFunction) {
		if($('#auto-close-dialogBox').length < 1) {
			$('body').append('<div id="auto-close-dialogBox"></div>');
		}		
